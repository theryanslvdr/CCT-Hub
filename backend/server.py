...
habit_router = APIRouter(prefix="/habits", tags=["Habit Tracker"])
...
@users_router.post("/push-subscribe")
...
@users_router.delete("/push-subscribe")
...
@users_router.get("/vapid-public-key")
...
@admin_router.post("/push-notify-all")
async def admin_push_notify_all(request: Request, user: dict = Depends(require_admin)):
...
async def create_license_invite(data: LicenseInviteCreate, user: dict = Depends(require_admin)):
    ...
    frontend_url = os.environ.get("FRONTEND_URL", "https://notify-admin-6.preview.emergentagent.com")
...
# ==================== BVE ROUTES (EXTRACTED) ====================
# Moved to routes/bve.py
...
# Habit tracker routes extracted to routes/habits.py
...
class AffiliateResourceCreate(BaseModel):
    category: str  # "conversation_starters", "story_templates", "marketing", "faqs"
    title: str
    content: str
    order: int = 0

@admin_router.get("/affiliate-resources")
...
@admin_router.post("/affiliate-resources")
...
@admin_router.put("/affiliate-resources/{resource_id}")
...
@admin_router.delete("/affiliate-resources/{resource_id}")
...
# Public endpoint for members
@api_router.get("/affiliate-resources")
...
# Chatbase config
@admin_router.get("/affiliate-chatbase")
...
@admin_router.put("/affiliate-chatbase")
...
@api_router.get("/affiliate-chatbase-public")
...
async def get_activity_feed(since: str = "", limit: int = 50, user: dict = Depends(require_admin)):
    ...
    query_filter = {}
    if since:
        query_filter["completed_at"] = {"$gt": since}
