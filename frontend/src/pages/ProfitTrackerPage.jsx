...
export const ProfitTrackerPage = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Undo Trade Confirmation Dialog */}
      <Dialog open={undoTradeDialogOpen} onOpenChange={setUndoTradeDialogOpen}>
        <DialogContent className="glass-card border-zinc-800 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-white flex items-center gap-2">
              <RotateCcw className="w-5 h-5 text-red-400" /> Undo Trade
            </DialogTitle>
          </DialogHeader>
          {undoTradeDate && (
            <div className="space-y-4 py-4">
              <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/30">
                <p className="text-red-400 text-sm">
                  ...
                  Are you sure you want to undo the trade for <span className="font-medium text-white">{undoTradeDate.dateStr}</span>?
                </p>
                <p className="text-zinc-400 text-xs mt-2">
                  This will delete the trade record and remove the profit ({formatMoney(undoTradeDate.actualProfit)}) from your account.
                </p>
              </div>
              ...
              <div className="flex justify-end gap-2">
                <Button
                  variant="outline"
                  onClick={() => setUndoTradeDialogOpen(false)}
                  className="border-zinc-700"
                  data-testid="cancel-undo-trade"
                >
                  Cancel
                </Button>
                <Button
                  onClick={handleUndoTrade}
                  disabled={undoTradeLoading}
                  className="bg-red-600 hover:bg-red-700 text-white"
                  data-testid="confirm-undo-trade"
                >
                  {undoTradeLoading ? 'Undoing...' : 'Undo Trade'}
                </Button>
              </div>
            </div>
          )}
      ...
      {/* Mark as Holiday Dialog */}
      <Dialog open={holidayDialogOpen} onOpenChange={setHolidayDialogOpen}>
        <DialogContent className="glass-card border-zinc-800 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-white flex items-center gap-2">
              <Calendar className="w-5 h-5 text-amber-400" /> Mark as Holiday
            </DialogTitle>
          </DialogHeader>
          {holidayDate && (
            <div className="space-y-4 py-4">
              <p className="text-zinc-400 text-sm">
                Mark <span className="text-white font-medium">{holidayDate.dateStr}</span> as a personal holiday.
              </p>
              <p className="text-zinc-500 text-xs">
                This day will be excluded from your trading projections and won't count as a missed trade.
              </p>
              ...
              <div className="flex justify-end gap-2">
                <Button
                  ...
                  className="border-zinc-700"
                  data-testid="cancel-mark-holiday"
                >
                  Cancel
                </Button>
                <Button
                  ...
                  data-testid="confirm-mark-holiday"
                >
